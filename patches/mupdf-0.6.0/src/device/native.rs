...
// On Windows (MSVC), bindgen does not generate max_align_t because MSVC
// headers handle it differently.  The max fundamental alignment on
// x86_64 MSVC is 8 bytes (same as f64 / i64).  Define a stand-in so
// the alignment assertion in create() compiles.
